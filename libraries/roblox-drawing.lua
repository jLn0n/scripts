...
drawing_container.Parent = core_gui --game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui", 15)
...
function drawing_lib.new(drawing_type)
	...
	elseif drawing_type == "Text" then
		...
		textLabel:GetPropertyChangedSignal("TextBounds"):Connect(function()
			...
			textLabel.Position = UDim2.fromOffset(drawing_info.Position.X + (if not drawing_info.Center then offset.X else 0), drawing_info.Position.Y + offset.Y)
		end)
		...
		return setmetatable(drawing_obj, {
			__newindex = function(_, index, value)
				...
				elseif index == "Position" then
					...
					textLabel.Position = UDim2.fromOffset(value.X + (if not drawing_info.Center then offset.X else 0), value.Y + offset.Y)
	...
	elseif drawing_type == "Circle" then
		...
		circleFrame.BackgroundTransparency = (if drawing_info.Filled then convert_dtransparency(drawing_info.Transparency) else 1)
		...
		return setmetatable(drawing_obj, {
			__newindex = function(_, index, value)
				...
				elseif index == "Filled" then
					circleFrame.BackgroundTransparency = (if value then convert_dtransparency(drawing_info.Transparency) else 1)
	...
	elseif drawing_type == "Square" then
		...
		squareFrame.BackgroundTransparency = (if drawing_info.Filled then convert_dtransparency(drawing_info.Transparency) else 1)
		...
		return setmetatable(drawing_obj, {
			__newindex = function(_, index, value)
				...
				elseif index == "Thickness" then
					...
					uiStroke.Thickness = value
				elseif index == "Filled" then
					squareFrame.BackgroundTransparency = (if value then convert_dtransparency(drawing_info.Transparency) else 1)
					uiStroke.Enabled = not value
				elseif index == "Visible" then
